(() => {
  ...
  // Build the deal from a valid removal sequence. This guarantees every new game
  // has at least one complete solution instead of relying on a lucky random deal.
  function makeSolvableDeck(){
    const sim=positions.map((p,i)=>({...p,removed:false,order:i}));
    const removal=[];
    while(removal.length<sim.length){
      const free=sim.filter(t=>!t.removed&&isFree(t,sim));
      if(free.length<2)return shuffle(types.map(t=>({...t})));
      const a=free[Math.floor(Math.random()*free.length)];
      const candidates=free.filter(t=>t!==a);
      const b=candidates[Math.floor(Math.random()*candidates.length)];
      a.removed=true;b.removed=true;removal.push(a.order,b.order);
    }
    const pairTypes=[];
    for(const t of types.filter((v,i,a)=>a.findIndex(x=>JSON.stringify(x)===JSON.stringify(v))===i)){
      // Every standard tile identity occurs four times, forming two pairs.
      if(t.kind!=='special')pairTypes.push({...t},{...t});
    }
    ...
    for(let i=0;i<72;i++){const type=pairTypes[i];deck[removal[i*2]]=({...type});deck[removal[i*2+1]]=({...type});}
    return deck;
  }

  function render(){
    board.innerHTML='';
    tiles.forEach(t=>{if(t.removed)return;
  ...
  function clickTile(t){
    ...
    if(same(selected.data,t.data)){
      ...
      if(pairs===72)finish();
      else if(!tiles.some(x=>!x.removed&&isFree(x)))flash('No open pairs remain — shuffle to continue.');
  ...
  function start(){
    clearInterval(timer);started=true;startTime=Date.now();timer=setInterval(tick,1000);moves=0;pairs=0;selected=null;update();
    const deck=makeSolvableDeck();tiles=positions.map((p,i)=>({...p,data:deck[i],order:i,removed:false}));
    modal.classList.add('hidden');messageEl.classList.add('hidden');timeEl.textContent='00:00';render();
  }
  ...
  function hint(){
    if(!started){flash('Start a game first.');return;}
    const free=tiles.filter(t=>!t.removed&&isFree(t));
    for(let i=0;i<free.length;i++)for(let j=i+1;j<free.length;j++)if(same(free[i].data,free[j].data)){
      selected=free[i];render();setTimeout(()=>{if(!started)return;selected=free[j];render();setTimeout(()=>{selected=null;render();},650);},650);return;
    }
    flash('No matching open pair found. Shuffle to continue.');
  }
