(() => {
  ...
  // A fixed valid peel order is used to deal the board. Each consecutive pair
  ...
  function clickTile(t){
    ...
    if(same(selected.data,t.data)){const a=selected,b=t;selected=null;a.removed=true;b.removed=true;moves++;pairs++;update();render();beep(620,.07);if(pairs===72)finish();else if(!tiles.some(x=>!x.removed&&isFree(x)))flash('No open pairs remain — shuffle to continue.');}
  ...
  function start(){clearInterval(timer);started=true;startTime=Date.now();timer=setInterval(tick,1000);moves=0;pairs=0;selected=null;update();const deck=makeSolvableDeck();tiles=positions.map((p,i)=>({...p,data:deck[i],order:i,removed:false}));modal.classList.add('hidden');messageEl.classList.add('hidden');timeEl.textContent='00:00';render();}
  ...
  $('startGame').addEventListener('click',start);$('playAgain').addEventListener('click',start);$('shuffle').addEventListener('click',shuffleRemaining);$('hint').addEventListener('click',hint);$('soundBtn').addEventListener('click',()=>{soundOn=!soundOn;$('soundBtn').textContent=soundOn?'🔊':'🔇';});
  document.addEventListener('keydown',e=>{if(e.key.toLowerCase()==='h')hint();if(e.key==='Escape'){selected=null;render();}});update();
})();