(() => {
  ...
  // In Mahjong Solitaire, any flower matches any flower, and any season matches any season.
  ...
  function clickTile(t){if(!started)return;if(!isFree(t)){flash('That tile is blocked.');return;}if(selected===t){selected=null;render();return;}if(!selected){selected=t;render();return;}if(same(selected.data,t.data)){const a=selected,b=t;selected=null;a.removed=true;b.removed=true;moves++;pairs++;update();render();beep(620,.07);if(pairs===72)finish();else if(!tiles.some(x=>!x.removed&&isFree(x)))flash('No open pairs remain — shuffle to continue.');}else{selected=t;flash('Those tiles do not match.');render();beep(180,.08);}}
